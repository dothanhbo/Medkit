...
import 'package:scr/src/features/screens/login/code_for_reset_password.dart';
...
Future checkEmail(BuildContext context, String email) async {
  final uri = Uri.parse(
      'http://ronalbo2610-001-site1.ftempurl.com/api/User/checkemail?email=$email');
  final headers = {'Content-Type': 'application/json'};
  ...
  if (response.statusCode == 200) {
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
          builder: (_) => CodeForResetPassword(user: user)),
    );
    Fluttertoast.showToast(
...
Future changePassword(BuildContext context, String id, String password, String confirmPassword) async {
  final uri = Uri.parse('http://ronalbo2610-001-site1.ftempurl.com/api/User/changePassword?id=$id');
  final headers = {'Content-Type': 'application/json'};
  ...
  if (response.statusCode == 200) {
    ...
    // Handle other error cases here.
    Fluttertoast.showToast(
      msg: "Password change failed: ${response.statusCode}",
    );
  }
}
